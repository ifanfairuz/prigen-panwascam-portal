...
export default function FormAGeneratorPage() {
  ...
  const genFormA = (type: DocType) => {
    setLoading(true);
    formA.genFormA(type).finally(() => setLoading(false));
  };
    ...
    <Container>
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-end gap-2">
          <button
            disabled={loading}
            onClick={() => genFormA("docx")}
            className="bg-blue-400 hover:bg-blue-500 text-blue-800 px-4 py-2 rounded-full text-sm md:text-md"
          >
            {loading && (
              <svg
                aria-hidden="true"
            ...
            {loading ? "Loading..." : "GENERATE DOCX"}
          </button>
          <button
            disabled={loading}
            onClick={() => genFormA("pdf")}
            className="bg-red-400 hover:bg-red-500 text-red-800 px-4 py-2 rounded-full text-sm md:text-md"
          >
            {loading && (
              <svg
                aria-hidden="true"

  ...
  const genFormA = () => {
    ...
    for (const key of mustCheck) {
      if (isEmpty((value as any)[key])) {
        (document.querySelector('input[name="' + key + '"]') as any)?.focus();
        return;
      }
    }
    if (!value.ttd) {
      (document.querySelector('input[name="ttd"]') as any)?.focus();
      return;
    }
    if ((value.dokumentasi?.filter((d) => !!d).length || 0) < 1) {
      (document.querySelector('input[name="dokumentasi"]') as any)?.focus();
      return;
    }

    getTemplate().then(async (buf) => {
